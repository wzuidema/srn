...
weights_hid_out = create_weights_hid_out()
...
class Layer(object):
	...
	def calc_activations(self):
		self.calc_net()
		for i in range(len(self.weighted_sum_list)):
			self.weighted_sum_list[i] = 1/(1+exp(-self.weighted_sum_list[i]))
...
class Hidden_Layer(Layer):
	def __init__(self, synapses_in, synapses_con, weights_in_hid, weigths_con_hid, weights_hid_out):
		...
		self.weights_hid_out = weights_hid_out		
	def calc_net(self):
	...
	def calc_error(self):
		...
		downstream_error_matrix = deepcopy(self.weights_hid_out)
...
class Output_Layer(Layer):
	def __init__(self, synapses_hid, weights_hid_out):
		self.synapses = synapses_hid
		self.weights = weights_hid_out
		self.calc_activations()
	def calc_error(self):
		error_list = []
		error = 0
		for i in range(len(self.activations)):
			error = self.activations[i]*(1-self.activations[i])*(target[i]-self.activations[i])
...
hidden_layer = Hidden_Layer(init_input_vector, init_synapses_con, weights_in_hid, weights_con_hid, weights_hid_out)

hidden_layer.calc_activations()
hidden_layer.add_bias()

context_layer = Context_Layer(hidden_layer.activations, weights_hid_con)
		
output_layer = Output_Layer(hidden_layer.activations, weights_hid_out)
...
while sweep < sweeps_train:
	cycle = 0
	cycle_plus_one = 1	
	while cycle_plus_one < (calc_length(training_set)-1):
		...
		target = synapses_in_list[cycle_plus_one]
...
#		print output_layer.weights
		...
		hidden_layer = Hidden_Layer(input_layer.activations, context_layer.activations, hidden_layer.weights_in_hid, hidden_layer.weights_con_hid, output_layer.weights)
		...
		output_layer = Output_Layer(hidden_layer.activations, output_layer.weights)
	...
	output_activations_matrix.append("end_of_sweep")	
	sweep += 1
...
while sweep < sweeps_test:
	cycle = 0
	cycle_plus_one = 1	
	while cycle_plus_one < ((len(synapses_in_list))-1):
	
		print ""
		print "This is the testing loop!!: "+str(cycle_plus_one)+" of sweep: "+str(sweep)
		print ""
		print ""
...
#		print output_layer.weights
	...
	output_activations_test.append("end_of_sweep")	
	sweep += 1
